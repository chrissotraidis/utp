...
sdl2-shared-ios: prepare-sdl2-source
	@xcodebuild -project build/sources/SDL2-UT99/Xcode/SDL/SDL.xcodeproj -scheme 'Shared Library-iOS' -sdk iphoneos -configuration Release -derivedDataPath build/sdl2-shared-ios CODE_SIGNING_ALLOWED=NO EXCLUDED_SOURCE_FILE_NAMES=SDL_shaders_metal.metal OTHER_CFLAGS='$$(inherited) -ffile-prefix-map=$(realpath build)=build' OTHER_CPLUSPLUSFLAGS='$$(inherited) -ffile-prefix-map=$(realpath build)=build' build

sdl2-shared-sim-ios: prepare-sdl2-source
	@xcodebuild -project build/sources/SDL2-UT99/Xcode/SDL/SDL.xcodeproj -scheme 'Shared Library-iOS' -sdk iphonesimulator -configuration Release -derivedDataPath build/sdl2-shared-sim-ios CODE_SIGNING_ALLOWED=NO EXCLUDED_SOURCE_FILE_NAMES=SDL_shaders_metal.metal build

ios-audio-deps:
	@./tools/build_ios_dependencies.sh

ios-audio-sim:
	@UT99_SDK=iphonesimulator UT99_DEP_SUFFIX=ios-sim UT99_MIN_FLAG=-mios-simulator-version-min=17.0 ./tools/build_ios_dependencies.sh

ios-desktop-shim:
	@set -eu; mkdir -p build/ios-engine build/ios-engine/deps; SDKROOT="$$(xcrun --sdk iphoneos --show-sdk-path)"; xcrun clang -dynamiclib -arch arm64 -isysroot "$$SDKROOT" -miphoneos-version-min=17.0 -fobjc-arc -fblocks -fvisibility=default -install_name @rpath/UT99DesktopShim.dylib Sources/UT99Runtime/UT99DesktopShim.c -framework CoreFoundation -o build/ios-engine/UT99DesktopShim.dylib; xcrun clang -dynamiclib -arch arm64 -isysroot "$$SDKROOT" -miphoneos-version-min=17.0 -fobjc-arc -fblocks -fvisibility=default -install_name @rpath/UT99MetalShim.dylib Sources/UT99Runtime/UT99MetalShim.m -framework Foundation -framework Metal -framework QuartzCore -Wl,-reexport_framework,Metal -o build/ios-engine/UT99MetalShim.dylib; cp build/ios-engine/UT99MetalShim.dylib build/ios-engine/deps/UT99MetalShim.dylib; for name in UT99CocoaShim UT99ApplicationServicesShim UT99CoreServicesShim; do cp build/ios-engine/UT99DesktopShim.dylib "build/ios-engine/$$name.dylib"; install_name_tool -id "@rpath/$$name.dylib" "build/ios-engine/$$name.dylib"; codesign --force --sign - "build/ios-engine/$$name.dylib" >/dev/null; cp "build/ios-engine/$$name.dylib" "build/ios-engine/deps/$$name.dylib"; done

ios-desktop-shim-sim:
	@set -eu; mkdir -p build/ios-engine-ios-sim build/ios-engine-ios-sim/deps; SDKROOT="$$(xcrun --sdk iphonesimulator --show-sdk-path)"; xcrun clang -dynamiclib -arch arm64 -isysroot "$$SDKROOT" -mios-simulator-version-min=17.0 -fobjc-arc -fblocks -fvisibility=default -install_name @rpath/UT99DesktopShim.dylib Sources/UT99Runtime/UT99DesktopShim.c -framework CoreFoundation -o build/ios-engine-ios-sim/UT99DesktopShim.dylib; xcrun clang -dynamiclib -arch arm64 -isysroot "$$SDKROOT" -mios-simulator-version-min=17.0 -fobjc-arc -fblocks -fvisibility=default -install_name @rpath/UT99MetalShim.dylib Sources/UT99Runtime/UT99MetalShim.m -framework Foundation -framework Metal -framework QuartzCore -Wl,-reexport_framework,Metal -o build/ios-engine-ios-sim/UT99MetalShim.dylib; cp build/ios-engine-ios-sim/UT99MetalShim.dylib build/ios-engine-ios-sim/deps/UT99MetalShim.dylib; for name in UT99CocoaShim UT99ApplicationServicesShim UT99CoreServicesShim; do cp build/ios-engine-ios-sim/UT99DesktopShim.dylib "build/ios-engine-ios-sim/$$name.dylib"; install_name_tool -id "@rpath/$$name.dylib" "build/ios-engine-ios-sim/$$name.dylib"; codesign --force --sign - "build/ios-engine-ios-sim/$$name.dylib" >/dev/null; cp "build/ios-engine-ios-sim/$$name.dylib" "build/ios-engine-ios-sim/deps/$$name.dylib"; done

ios-fmod-stub:
	@mkdir -p build/ios-engine/deps; SDKROOT="$$(xcrun --sdk iphoneos --show-sdk-path)"; xcrun clang -dynamiclib -arch arm64 -isysroot "$$SDKROOT" -miphoneos-version-min=17.0 -fvisibility=default -install_name @rpath/libfmod.dylib Sources/UT99Runtime/UT99FMODShim.c -o build/ios-engine/deps/libfmod.dylib; codesign --force --sign - build/ios-engine/deps/libfmod.dylib >/dev/null

ios-fmod-stub-sim:
	@mkdir -p build/ios-engine-ios-sim/deps; SDKROOT="$$(xcrun --sdk iphonesimulator --show-sdk-path)"; xcrun clang -dynamiclib -arch arm64 -isysroot "$$SDKROOT" -mios-simulator-version-min=17.0 -fvisibility=default -install_name @rpath/libfmod.dylib Sources/UT99Runtime/UT99FMODShim.c -o build/ios-engine-ios-sim/deps/libfmod.dylib; codesign --force --sign - build/ios-engine-ios-sim/deps/libfmod.dylib >/dev/null

ios-fmod-real: ios-desktop-shim
	@set -eu; mkdir -p build/ios-engine/deps; source="build/macos-baseline/UnrealTournament.app/Contents/Frameworks/libfmod.dylib"; thin="build/ios-engine/deps/libfmod-macos-arm64.dylib"; output="build/ios-engine/deps/libfmod.dylib"; test -f "$$source"; lipo -thin arm64 "$$source" -output "$$thin"; xcrun vtool -set-build-version ios 17.0 26.5 -replace -output "$$output" "$$thin"; install_name_tool -id @rpath/libfmod.dylib "$$output"; install_name_tool -add_rpath @loader_path "$$output"; install_name_tool -change /System/Library/Frameworks/Carbon.framework/Versions/A/Carbon @rpath/UT99DesktopShim.dylib "$$output"; install_name_tool -change /System/Library/Frameworks/AudioUnit.framework/Versions/A/AudioUnit /System/Library/Frameworks/AudioToolbox.framework/AudioToolbox "$$output"; install_name_tool -change /System/Library/Frameworks/CoreAudio.framework/Versions/A/CoreAudio /System/Library/Frameworks/CoreAudio.framework/CoreAudio "$$output"; install_name_tool -change /System/Library/Frameworks/CoreFoundation.framework/Versions/A/CoreFoundation /System/Library/Frameworks/CoreFoundation.framework/CoreFoundation "$$output"; codesign --force --sign - "$$output" >/dev/null; rm -f "$$thin"; file "$$output"; otool -L "$$output"

ios-fmod-real-sim: ios-desktop-shim-sim
	@set -eu; mkdir -p build/ios-engine-ios-sim/deps; source="build/macos-baseline/UnrealTournament.app/Contents/Frameworks/libfmod.dylib"; thin="build/ios-engine-ios-sim/deps/libfmod-macos-arm64.dylib"; output="build/ios-engine-ios-sim/deps/libfmod.dylib"; test -f "$$source"; lipo -thin arm64 "$$source" -output "$$thin"; xcrun vtool -set-build-version iossim 17.0 26.5 -replace -output "$$output" "$$thin"; install_name_tool -id @rpath/libfmod.dylib "$$output"; install_name_tool -add_rpath @loader_path "$$output"; install_name_tool -change /System/Library/Frameworks/Carbon.framework/Versions/A/Carbon @rpath/UT99DesktopShim.dylib "$$output"; install_name_tool -change /System/Library/Frameworks/AudioUnit.framework/Versions/A/AudioUnit /System/Library/Frameworks/AudioToolbox.framework/AudioToolbox "$$output"; install_name_tool -change /System/Library/Frameworks/CoreAudio.framework/Versions/A/CoreAudio /System/Library/Frameworks/CoreAudio.framework/CoreAudio "$$output"; install_name_tool -change /System/Library/Frameworks/CoreFoundation.framework/Versions/A/CoreFoundation /System/Library/Frameworks/CoreFoundation.framework/CoreFoundation "$$output"; codesign --force --sign - "$$output" >/dev/null; rm -f "$$thin"; file "$$output"; otool -L "$$output"
...
ios-engine-real-package: ios-engine-real-artifact
	@UT99_ENGINE_EMBED=1 xcodebuild -project UT99Apple.xcodeproj -scheme UT99Apple -sdk iphoneos -configuration Debug -derivedDataPath build/ios-engine-real-app CODE_SIGNING_ALLOWED=NO build
...
ios-engine-sim-real-package: ios-engine-sim-real-artifact
	@UT99_ENGINE_EMBED=1 xcodebuild -project UT99Apple.xcodeproj -scheme UT99Apple -sdk iphonesimulator -configuration Debug -derivedDataPath build/ios-engine-sim-real-app CODE_SIGNING_ALLOWED=NO build
...
ios-engine-sim-package: ios-engine-sim-artifact
	@UT99_ENGINE_EMBED=1 xcodebuild -project UT99Apple.xcodeproj -scheme UT99Apple -sdk iphonesimulator -configuration Debug -derivedDataPath build/ios-engine-sim-app CODE_SIGNING_ALLOWED=NO build

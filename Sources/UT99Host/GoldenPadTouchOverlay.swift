...
final class GoldenPadTouchOverlay: UIView {
    ...
    enum Action: String, CaseIterable {
        case primaryFire, alternateFire, jump, crouch
    ...
    private let lookSurface = UIView()
    ...
    private func applyReferenceFrames(_ frames: [String: CGRect]) {
        ...
        if let frame = frames["look"] {
            placeStick(lookSurface, ring: lookSurface, key: "look",
                       center: CGPoint(x: frame.midX, y: frame.midY), diameter: frame.width)
        }
    ...
    private func placeStick(_ view: UIView, ring: UIView, key: String, center fallback: CGPoint, diameter: CGFloat) {
        ...
        view.bounds.size = CGSize(width: resolvedDiameter, height: resolvedDiameter)
        view.center = resolvedCenter
        if ring !== view { ring.frame = view.bounds }
        ...
        if view === movePad {
            ...
            lookThumb.center = CGPoint(x: lookSurface.bounds.midX, y: lookSurface.bounds.midY)
            lookThumb.layer.cornerRadius = thumbDiameter / 2
            let glyphSide = min(20, max(13, thumbDiameter * 0.42))
    ...
    private func configureSurface() {
        ...
        // gives the player proprioception that a roaming invisible zone cannot.
        ...
        lookThumb.backgroundColor = UIColor(red: 1.00, green: 0.84, blue: 0.25, alpha: 0.98)
        lookThumb.layer.cornerRadius = 100
        lookThumb.isUserInteractionEnabled = false
        lookSurface.addSubview(lookThumb)
    ...
    private func setButton(_ action: Action, center: CGPoint, size: CGSize, scale: CGFloat = 1) {
        ...
        button.isHidden = touchConfiguration.hiddenActions.contains(action.rawValue)
        let fallback = UT99TouchPlacement(x: center.x / max(bounds.width, 1), y: center.y / max(bounds.height, 1), scale: 1)
        let placement = placements[action.rawValue] ?? fallback
    ...
    private func center(forKey key: String, in size: CGSize, fallback: CGPoint) -> CGPoint {
        if let placement = placements[key] {
    ...
    private func updateLayoutModeAppearance() {
        ...
        movePad.layer.cornerRadius = movePad.bounds.width / 2
        lookSurface.backgroundColor = UIColor(red: 0.91, green: 0.66, blue: 0.08, alpha: 0.90)
        lookSurface.layer.borderWidth = editingLayout ? 3 : 2
        lookSurface.layer.borderColor = editingLayout
            ? UIColor(red: 0.35, green: 0.92, blue: 0.88, alpha: 0.96).cgColor
    ...
    @objc private func layoutPanChanged(_ gesture: UIPanGestureRecognizer) {
        ...
        var placement = placements[action.rawValue] ?? UT99TouchPlacement(x: x, y: y, scale: 1)
        placement.x = x
        placement.y = y
        placements[action.rawValue] = placement
        button.center = CGPoint(x: x * bounds.width, y: y * bounds.height)
        if gesture.state == .ended || gesture.state == .cancelled {
            NSLog("UT99 touch layout moved action=%@ x=%.3f y=%.3f", action.rawValue, placement.x, placement.y)
            savePlacements()
        }
    }
    ...
    @objc private func layoutPinchChanged(_ gesture: UIPinchGestureRecognizer) {
        ...
        var placement = placements[action.rawValue] ?? UT99TouchPlacement(
    ...
    @objc private func layoutZonePinchChanged(_ gesture: UIPinchGestureRecognizer) {
        ...
        let key = view === movePad ? "move" : "look"
        var placement = placements[key] ?? UT99TouchPlacement(
    ...
    @objc private func moveChanged(_ gesture: UIPanGestureRecognizer) {
        if editingLayout {
            let point = gesture.location(in: self)
            let halfWidth = movePad.bounds.width / 2
            let halfHeight = movePad.bounds.height / 2
            let x = min(max(point.x, halfWidth), bounds.width - halfWidth) / max(bounds.width, 1)
            let y = min(max(point.y, halfHeight), bounds.height - halfHeight) / max(bounds.height, 1)
            let scale = placements["move"]?.scale ?? 1
            placements["move"] = UT99TouchPlacement(x: x, y: y, scale: scale)
            movePad.center = CGPoint(x: x * bounds.width, y: y * bounds.height)
            if gesture.state == .ended || gesture.state == .cancelled { savePlacements() }
            return
        }
        ...
        let value = normalizedStickValue(
            point: point,
            center: anchor,
            radius: min(movePad.bounds.width, movePad.bounds.height) * 0.5,
            invertY: true
        )
        let travel = max(0, movePad.bounds.width * 0.5 - moveThumb.bounds.width * 0.5 - 4)
    ...
    @objc private func lookChanged(_ gesture: UIPanGestureRecognizer) {
        if editingLayout {
            let point = gesture.location(in: self)
            let halfWidth = lookSurface.bounds.width / 2
            let halfHeight = lookSurface.bounds.height / 2
            let x = min(max(point.x, halfWidth), bounds.width - halfWidth) / max(bounds.width, 1)
            let y = min(max(point.y, halfHeight), bounds.height - halfHeight) / max(bounds.height, 1)
            let scale = placements["look"]?.scale ?? 1
            placements["look"] = UT99TouchPlacement(x: x, y: y, scale: scale)
            lookSurface.center = CGPoint(x: x * bounds.width, y: y * bounds.height)
            if gesture.state == .ended || gesture.state == .cancelled { savePlacements() }
            return
        }
        let point = gesture.location(in: lookSurface)
        let center = CGPoint(x: lookSurface.bounds.midX, y: lookSurface.bounds.midY)
        let value = normalizedStickValue(
            point: point,
            center: center,
            radius: min(lookSurface.bounds.width, lookSurface.bounds.height) * 0.5,
            invertY: false
        )
        let travel = max(0, lookSurface.bounds.width * 0.5 - lookThumb.bounds.width * 0.5 - 4)
        lookThumb.transform = CGAffineTransform(translationX: value.x * travel,
                                                y: value.y * travel)
        onLook?(value, gesture.state != .ended && gesture.state != .cancelled)
        if gesture.state == .ended || gesture.state == .cancelled {
    ...
    private func title(for action: Action) -> String {
        switch action {
        case .primaryFire: "FIRE"
        case .alternateFire: "ALT"
        case .jump: "JUMP"
        case .use: "USE"
        case .crouch: "DUCK"
        case .nextWeapon: "NEXT"
        case .previousWeapon: "PREV"
    ...
    private func visualRole(for action: Action) -> UT99TouchActionButton.Role {
        ...
        case .pause: .start
        case .scoreboard, .use, .nextWeapon, .previousWeapon: .dPad
    ...
    private func accessibilityHint(for action: Action) -> String {
        switch action {
        case .primaryFire: "Shoots the current weapon"
    ...
    private func tint(for action: Action) -> UIColor {
        switch action {
        case .primaryFire: UIColor(red: 0.08, green: 0.56, blue: 0.29, alpha: 0.92)

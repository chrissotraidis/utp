...
| Refined iPhone/iPad touch UI | PARTIAL | Reference-derived UT mapping, current iPad/iPhone screenshots, non-overlapping UT action geometry, profiles, handedness, explicit simultaneous move/look recognizer arbitration, and live assistive FIRE pass. Internal reference names are absent from player settings, new exports, and current diagnostics; legacy IDs migrate silently. Physical two-thumb play, reach, haptics, VoiceOver stick traversal, and touch-only play require devices. |
...
| Server browser/master query | PARTIAL | The Simulator slice now passes through joining: point-accurate original UWindow input selects `UT Servers`, populates the stock list, double-clicks a row, completes the v469 challenge/download path, loads the remote map, and possesses the player. Real-finger/device networking and the full play/chat/transition/disconnect sequence remain open. |

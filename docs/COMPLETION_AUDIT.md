...
| First-run game-data acquisition | PARTIAL | The data-free app presents Get Game Data/Import Files, discloses source/size/terms before consent, retries OldUnreal/Archive mirrors, verifies the exact 649633792-byte SHA-256-pinned ISO, extracts only four data directories, omits incompatible fonts, installs transactionally, and cleans temporary media. Fixture and official-image extraction plus Simulator UI pass; physical download/storage/interruption behavior and public-release permission remain open. |
...
| Lifecycle and recovery | PARTIAL | Simulator background/foreground, single-engine invariant, crash marker, Safe Mode and diagnostics pass. A final-package iPhone run survived three Home/reopen cycles in one PID/engine, accepted post-resume FIRE, and traveled Deck16 → Codex during a 30-minute soak. Physical suspend, watchdog/OOM, controlled return, audio-route recovery, and share-sheet recovery remain. |
...
| Performance and stability | PHYSICAL-ONLY BLOCKED | FruCoRe presentation instrumentation works; a final-package iPhone Simulator run reported 121.88 produced FPS at 10 seconds on 2868×1320 and completed a 30:48 one-process soak with bounded RSS release dips. Physical FPS/pacing/thermals/memory pressure/battery and a hardware long run are required. |
...
| Public beta/distribution onboarding | IMPLEMENTED LOCALLY / PERMISSION BLOCKED | TestFlight-first beta and physical handoff are specified; the data-free first-run acquisition/import UX is implemented and Simulator-tested. Prebuilt transformed-engine distribution and enabling direct acquisition in a public binary still require explicit permission and Apple channel approval; a raw ad-hoc IPA is not a general public installer. |
...
| Runtime discipline | PROVEN | The final iPhone Simulator soak used exactly one Simulator and one UT99 client; all project runtimes are now stopped for physical-device handoff, and the EctoPad reference checkout is pristine at `461de17f549d98742bc3b2d031156f79ab3eaa9d`. |

...
| Refined iPhone/iPad touch UI | PARTIAL | Reference-derived UT mapping, current iPad/iPhone screenshots, geometry, profiles, handedness, and live assistive FIRE pass. Internal reference names are absent from player settings, new exports, and current diagnostics; legacy IDs migrate silently. Finger reach, simultaneous multi-touch, haptics, VoiceOver traversal, and touch-only play require devices. |

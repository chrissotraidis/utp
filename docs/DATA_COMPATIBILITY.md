...
Runtime imports no longer merge staged files directly into the live set. The host copies the complete last-known-good content/manifest into a backup, publishes an atomic phase journal, replaces only Maps/Music/Sounds/Textures plus the user manifest, and recovers an interrupted committing phase before startup. The generated System tree is never part of the transaction. Preparation runs outside UIKit's main queue and reports phase, current file, count, and progress. Cancellation is cooperative through download, ISO extraction, ZIP entries, copies, and 1 MiB SHA-256 chunks; it disables when the non-cancellable journaled install begins. Verify uses streaming SHA-256 reads and the Game Data menu exposes download, repair/reimport, and manifest export. Simulator interruption/replacement evidence is in `docs/evidence/engine-startup/2026-08-24-ios-data-transaction-pass/`, progress/cancellation evidence is in `docs/evidence/engine-startup/2026-08-24-ios-import-progress-pass/`, and first-run acquisition evidence is in `docs/evidence/onboarding/2026-08-25-first-run-acquisition/`; physical-device acquisition and Steam-origin imports remain open.
...
The candidate now implements a user-consented **Get Game Data** path matching that source contract: four pinned mirrors, exact byte count and SHA-256 verification, a bounded read-only ISO-9660/Joliet extractor, incompatible font omission, transactional install, and temporary-image cleanup. The IPA remains data-free and manual folder/ZIP import remains available. This implementation does not itself authorize a public binary to automate the download, mirror the image, or redistribute the transformed OldUnreal runtime; those remain release gates in `docs/DISTRIBUTION_AND_ONBOARDING.md`.

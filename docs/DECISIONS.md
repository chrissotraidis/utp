...
- **Decision:** Remove the preset selector and preset submenu from player UI. Keep legacy profile identifiers only in the versioned import/migration boundary, canonicalize them to `standard` before persistence/export, and use neutral UT99 wording in runtime diagnostics and device-verification copy. Present direct player controls for opacity, size, handedness, and controller auto-hide, use Arrange/Save/Restore language, and hide gameplay controls while settings own interaction.
- **Evidence:** `Sources/UT99Host/GameViewController.swift`; `Sources/UT99Host/GoldenPadTouchOverlay.swift`; `Sources/UT99Host/UT99TouchProfileStore.swift`; `Tests/TouchProfileStoreTests.swift`; `Tests/test_host_state_and_data_menu.sh`; `design-qa.md`; `docs/evidence/engine-startup/2026-08-24-ios-control-settings-copy-fix/`; `docs/evidence/engine-startup/2026-08-24-player-profile-language/RESULT.md`.
...
## 2026-08-24 — Adapt physical-button overlap into unambiguous UT touch targets

- **Context:** Static comparison showed that the latest controls retained the reference's materials, sizing, fixed sticks, and thumb hierarchy, but source inspection exposed two interaction problems: the reference's physical A/B/X/Y faces overlap while UT maps them to independent gameplay actions, and UIKit movement/look recognizers had no explicit simultaneous-recognition policy.
...
## 2026-08-25 — Separate app distribution from consent-based game-data acquisition

- **Context:** The local app is ready for physical promotion, OldUnreal now publishes installers that acquire UT99 GOTY media, and a simple install/data onboarding path would materially improve adoption. A diagnostic or Ad Hoc IPA is not a universal web installer, and source availability does not automatically grant this project redistribution rights for the transformed engine or game data.
- **Decision:** Use physical development signing first and prefer TestFlight for an external beta. Keep the app artifact and game data separate. Specify **Get Game Data** as an explicit-consent, approved-source, pinned-hash, data-only transactional import path alongside manual folder/ZIP import. Treat App Store and eligible-region Apple website distribution as permission/review-gated channels. Treat online play as launch-critical and prohibit public claims until physical networking plus observer validation pass.
- **Evidence:** `docs/DISTRIBUTION_AND_ONBOARDING.md`; OldUnreal full-game installer and v469e release documentation; Apple beta/release and website-distribution documentation; existing Simulator browser and meaningful-session evidence.
- **Reversal condition:** Written owner permission or Apple review guidance requires a different artifact/data boundary or distribution channel; preserve explicit consent, provenance, integrity verification, data-only imports, and honest online evidence.
